...
programming languages. The most notable high-level representation are programming languages themselves.
...
sets, the first form of a programming language was invented, assembly.
...
server-side scaffolding and client-side SDKs, enhancing API consistency and
reducing complexity.
...
construct an HTTP request or handle serialization and deserialization.
...
commits](https://github.com/llvm/llvm-project) for a good reason. Compilers
essentially treat written code as data so imagine how complex the [ASTs](https://en.wikipedia.org/wiki/Abstract_syntax_tree) and
program flows can be.
...
But these products introduce a new complexity. Microsoft even published a video
guide on ["Prompting with
Copilot"](https://www.youtube.com/watch?v=ImWfIDTxn7E), to help you be more
effective while using Copilot. One tip from the video is to write a comment like
`# Description: The file processes a JSON` at the top of a Python file to give
Copilot more context. Writing extraneous comments is not typical so developers
have to be aware of these nuances to properly prompt the AI.
...
[Magic](https://magic.dev/) recognizes this limitation and is on a mission to
solve it. But despite these complexities, AI shows a lot of immediate value when
properly used.
...
abstraction to build performant software](https://www.modular.com/mojo). From
low-level assembly to AI assistants, compilers and code generation has proven to
be a major productivity multiplier for software engineers. And with the most
recent advancements in AI, we find ourselves asking, "Is natural language the
final form of programming?". At Konfig, we leave the answer up to practical
experimentation. After all, if a rules-based engine can be outperformed by LLMs,
why spend thousands of engineering hours building one?

I believe completion-based assistants have strong fit in the software
development lifecycle, but LLMs just don't cut it when you need 100% accuracy.
Where I see AI falling short is when 100% accuracy is expected. Although [some
are choosing to go all-in on AI-based code
generation](https://blog.darklang.com/gpt/). But I am always open to the
possibility of being wrong and would happily delete our entire codebase if AI
